	  ...
	  if (nlh->nlmsg_type == RTM_NEWLINK)
	      ...
	      while (RTA_OK (rta, rtasize))
		{
		  ...
		  switch (rta->rta_type)
		    {
		    case IFLA_ADDRESS:
		      if (rta_payload <= sizeof (ifas[ifa_index].addr.sl.sll_addr))
			{
		    ...
		    case IFLA_BROADCAST:
		      if (rta_payload <= sizeof (ifas[ifa_index].broadaddr.sl.sll_addr))
			{
		      ...
		      if ((rta_payload + 1) <= sizeof (ifas[ifa_index].name))
			{
			  ...
			  *(char *) memcpy (ifas[ifa_index].name, rta_data,rta_payload);
		...
		ifas[ifa_index - 1].ifa.ifa_next = &ifas[ifa_index].ifa;
	      ...
	      while (RTA_OK (rta, rtasize))
		{
		  unsigned char *rta_data = RTA_DATA (rta);
		  size_t rta_payload = RTA_PAYLOAD (rta);
...
#define IN6_IS_ADDR_MC_LINKLOCAL(a) \
	(IN6_IS_ADDR_MULTICAST(a)					      \
	 && ((((__const uint8_t *) (a))[1] & 0xf) == 0x2))
			  ...
			  default:
			    if (rta_payload <= sizeof (sa->sa_data))
			...
			default:
			  if (rta_payload <= sizeof (ifas[ifa_index].addr.sa.sa_data))
			...
			default:
			  if (rta_payload <= sizeof (ifas[ifa_index].broadaddr.sa.sa_data))
